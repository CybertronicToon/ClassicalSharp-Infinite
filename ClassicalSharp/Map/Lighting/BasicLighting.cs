...
namespace ClassicalSharp.Map {
	...
	public sealed partial class BasicLighting : IWorldLighting {
		...
		public override void Init(Game game) {
			this.game = game;
			this.ChunkHandler = game.World.ChunkHandler;
		...
		public unsafe override void LightHint(int startX, int startZ) {
			...
			if (ChunkHandler == null) ChunkHandler = game.World.ChunkHandler;
			
			int elemsLeft = InitialHeightmapCoverage(x1, z1, xCount, zCount, skip);
			if (!CalculateHeightmapCoverage_2(x1, z1, xCount, zCount, elemsLeft, skip)) {
		...
		public override int LightCol_YTop_Fast(int x, int y, int z) {
			if (y < game.World.MaxY) {
				...
				BlockID curBlock = game.World.ChunkHandler.GetBlock(x, y + 1, z);
